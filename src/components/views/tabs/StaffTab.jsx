  ...
  return (
    <div className="flex flex-col h-full min-h-0 overflow-hidden bg-white">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-6 py-4 space-y-5">
        ...
        <div className="rounded-2xl bg-[#0b3860] p-4 text-white shadow-sm">
          ...
          <button onClick={async () => { setSettling(true); await settleStaffLedger(staff.id, unsettledIds); setSettling(false); }}
            ...
            className="w-full py-2.5 rounded-xl bg-white/15 hover:bg-white/25 text-[12px] font-bold transition disabled:opacity-40 flex items-center justify-center gap-1.5 cursor-pointer active:scale-[0.98]">
        ...
        <div className="flex gap-2.5">
          <button onClick={() => setShowAdvance(true)}
            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-[#2270b8] text-[#2270b8] bg-[#2270b8]/5 hover:bg-[#2270b8]/10 text-[12px] font-bold transition cursor-pointer active:scale-[0.98]">
            <Banknote size={14} /> Cash Advance
          </button>
          <button onClick={() => setShowErrand(true)}
            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-zinc-200 text-zinc-700 bg-zinc-50 hover:bg-zinc-100 text-[12px] font-bold transition cursor-pointer active:scale-[0.98]">
        ...
        <div>
          ...
          <div className="space-y-2">
            {loading ? (
              <div className="py-6 text-center text-zinc-400 text-[13px]">Loading transactions…</div>
            ) : errands.length === 0 ? (
              <div className="py-8 text-center text-[13px] text-zinc-400 border border-dashed border-zinc-200 rounded-xl bg-zinc-50/50">
        ...
        {/* Assigned Maintenance Tasks Section */}
        <div className="pt-2 border-t border-zinc-100">
          ...
          <div className="space-y-2">
            {assignedTickets.length === 0 ? (
              <div className="py-6 text-center text-[13px] text-zinc-400 border border-dashed border-zinc-200 rounded-xl bg-zinc-50/50">
                No maintenance tasks currently assigned to {staff.name.split(' ')[0]}
              </div>
            ) : assignedTickets.map(t => (
              <div key={t.id} className="group rounded-xl border border-zinc-200 bg-white p-3 flex items-start gap-3 shadow-xs hover:border-zinc-300 transition">
